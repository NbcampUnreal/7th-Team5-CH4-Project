...
void ADDLobbyGameMode::ProcessPlayerJoin(ADDLobbyPlayerController* LobbyPlayerController, const FString& Nickname)
{
	...
	if (ADDBasePlayerState* BasePlayerState = LobbyPlayerController->GetPlayerState<ADDBasePlayerState>())
	{
		...
		if (Participants.Num() < GameStateBase->MinPlayerCount)
		{
			...
			LOG_CJH(Log, TEXT("[%s]님이 게임에 접속하셨습니다. 게임에 필요한 인원 수: %d/%d"), *Nickname, Participants.Num() + 1, GameStateBase->MinPlayerCount);
		...
		else
		{
			...
			LOG_CJH(Log, TEXT("[%s]님이 관전자로 접속하셨습니다."), *Nickname);
...
void ADDLobbyGameMode::Logout(AController* Exiting)
{
	...
	{
		LOG_CJH(Log, TEXT("[%s]님이 게임을 종료하셨습니다."), *BasePlayerState->Nickname);
